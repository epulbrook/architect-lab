...
const CompetitorWaitlist = () => {
  const [submitted, setSubmitted] = useState(false);
  const [formData, setFormData] = useState({ email: '', company: '', website: '' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const fixedWebsite = fixUrl(formData.website);
    const data = new FormData();
    data.append('email', formData.email);
    data.append('company', formData.company);
    data.append('website', fixedWebsite);
    data.append('tool_interest', 'Competitor Website Checker');
    try {
    ...
    <div>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: radial-gradient(circle at 30% 20%, rgba(0, 0, 0, 0.02) 0%, transparent 50%); pointer-events: none; }
        ...
        .form-group input { width: 100%; padding: 16px 20px; background: rgba(255, 255, 255, 0.9); border: 1px solid rgba(0, 0, 0, 0.1); border-radius: 12px; color: #1a1a1a; font-size: 1rem; transition: all 0.3s ease; }
        .form-group input:focus { outline: none; border-color: rgba(0, 0, 0, 0.3); background: rgba(255, 255, 255, 1); box-shadow: 0 0 0 3px rgba(0, 0, 0, 0.05); }
          ...
          <div className="hero-content">
            <div className="hero-text">
              ...
              <p className="subheadline">
                Because wondering if your website's rubbish compared to everyone else's is keeping you up at night.
              </p>
              <div className="features-grid">
                <div className="feature-item">
                  <div className="feature-number">01</div>
                  <div className="feature-title">The Uncomfortable Truth</div>
                  <div className="feature-desc">Find out exactly where your site's letting you down (spoiler: it's probably the loading speed)</div>
                </div>
                <div className="feature-item">
                  <div className="feature-number">02</div>
                  <div className="feature-title">SEO Reality Check</div>
                  <div className="feature-desc">Discover why you're invisible on Google whilst your competitors are living their best search life</div>
                </div>
                <div className="feature-item">
                  <div className="feature-number">03</div>
                  <div className="feature-title">Actual Fixes</div>
                  <div className="feature-desc">Not just problems—proper solutions you can actually implement without a PhD in web dev</div>
                </div>
              </div>
            </div>
            <div className="form-card">
              <div className="form-header">
                ...
                <p className="form-subtitle">Skip the queue, avoid the awkward launch bugs</p>
              </div>
              {!submitted ? (
                <form onSubmit={handleSubmit} id="competitorForm" autoComplete="off">
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" id="website" name="website" placeholder="yourwebsite.com" value={formData.website} onChange={handleChange} required autoComplete="off" />
                  </div>
                  <button type="submit" className="submit-btn" disabled={loading}>{loading ? 'Submitting...' : 'Join Waitlist'}</button>
                </form>
              ) : (
                <div className="trust-note">Cheers! You're on the list.</div>
              )}
...
export default CompetitorWaitlist; 